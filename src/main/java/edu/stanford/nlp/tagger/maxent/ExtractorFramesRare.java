...
import com.wantedtech.common.xpresso.sentence.pos.en.stanford.ASBCunkDict;
import com.wantedtech.common.xpresso.sentence.pos.en.stanford.CTBunkDict;
import com.wantedtech.common.xpresso.sentence.pos.en.stanford.CtbDict;
import com.wantedtech.common.xpresso.sentence.pos.en.stanford.Dictionary;
import com.wantedtech.common.xpresso.sentence.pos.en.stanford.FrenchUnknownWordSignatures;
import com.wantedtech.common.xpresso.sentence.pos.en.stanford.SpanishUnknownWordSignatures;
...
protected static Extractor[] getExtractorFramesRare(String identifier, TTags ttags) {
 ...
 List<String> args = valueSplit(identifier, "[a-zA-Z0-9]*(?:\\([^)]*\\))?", "\\s*,\\s*");
...
public CompanyNameDetector() {
 companyNameEnds = Helpers.newHashSet();
...
public String toString() {
 return x.Object(this).getShortClassName() + "(len" + num + ",w" + position + ")";
}
...
@Override
public String toString() {
 return x.Object(this).getShortClassName() + '(' + extractor1 + ',' + extractor2 + ')';
}
...
@Override
String extract(History h, PairsHolder pH) {
 String s = MaxentTagger.toNice(pH.getWord(h, position));
...
@Override
String extract(History h, PairsHolder pH) {
 String s=MaxentTagger.toNice(pH.getWord(h,n1));
