...
import com.wantedtech.common.xpresso.sentence.pos.en.stanford.WordShapeClassifier;
...
protected static Extractor[] getExtractorFrames(String arch) {
  ...
  List<String> args = valueSplit(arch, "[a-zA-Z0-9]*(?:\\([^)]*\\))?", "\\s*,\\s*");
  for (String arg : args) {
	...
	if (rightOrder < 0) { throw new IllegalArgumentException("Right order must be non-negative, not " + rightOrder); }
      ...
      extrs.add((Extractor)new ExtractorVerbalVBNZero(order));
