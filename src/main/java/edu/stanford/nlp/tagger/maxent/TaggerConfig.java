...
public class TaggerConfig extends Properties /* Inherits implementation of Serializable! */ {
  ...
  private static InputStream findStreamInClasspathOrFileSystem(String name) throws FileNotFoundException {
    ...
    if (is == null) {
      is = Helpers.class.getClassLoader().getResourceAsStream(name.replaceAll("\\\\", "/"));
      // Classpath doesn't like double slashes (e.g., /home/user//foo.txt)
      if (is == null) {
        is = Helpers.class.getClassLoader().getResourceAsStream(name.replaceAll("\\\\", "/").replaceAll("/+", "/"));
      }
    }
    ...
    if (! props.containsKey("trainFile")) {
      ...
      if (name != null) {
        try {
          ...
          DataInputStream in = new DataInputStream(getInputStreamFromURLOrClasspathOrFileSystem(name));
          this.putAll(TaggerConfig.readConfig(in)); // overwrites defaults with any serialized values.
          in.close();
        } catch (Exception e) {
          throw new RuntimeException("No such trained tagger config file found: " + name);
        }
      }
    }
  ...
  public static Properties argsToProperties(String[] args, Map<String,Integer> flagsToNumArgs) {
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      if (key.length() > 0 && key.charAt(0) == '-') { // found a flag
        ...
        if (flagArgs.isEmpty()) {
          ...
          result.setProperty(key, x.String(" ").join(flagArgs));
          if (key.equalsIgnoreCase(PROP) || key.equalsIgnoreCase(PROPS) || key.equalsIgnoreCase(PROPERTIES) || key.equalsIgnoreCase(ARGUMENTS) || key.equalsIgnoreCase(ARGS)) {
            try {
              BufferedReader reader = readerFromString(result.getProperty(key));
    ...
    if (!remainingArgs.isEmpty()) {
      result.setProperty("", x.String(" ").join(remainingArgs));
    }
