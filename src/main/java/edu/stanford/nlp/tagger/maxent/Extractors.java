...
public class Extractors implements Serializable {
  ...
  transient List<tuple2<Integer,Extractor>>
  ...
  void initTypes() {
    ...
    local = new ArrayList<tuple2<Integer,Extractor>>();
    localContext = new ArrayList<tuple2<Integer,Extractor>>();
    dynamic = new ArrayList<tuple2<Integer,Extractor>>();
    ...
    for(int i=0; i<v.length; ++i) {
      ...
      if(e.isLocal()) {
        local.add(tuple2.valueOf(i,e));
        //localContext.put(i,e);
      } else if(e.isDynamic()) {
        dynamic.add(tuple2.valueOf(i,e));
      } else {
        localContext.add(tuple2.valueOf(i,e));
      }
    }
    if(DEBUG) {
