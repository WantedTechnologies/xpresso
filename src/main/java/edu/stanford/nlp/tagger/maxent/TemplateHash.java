...
public class TemplateHash {
  ...
  private final Map<tuple2<Integer,String>,ListInstances> tempHash = new HashMap<tuple2<Integer,String>,ListInstances>();
  ...
  protected void addPositions(int start, int end, FeatureKey fK) {
    tuple2<Integer, String> key = tuple2.valueOf(fK.num, fK.val);
    tempHash.get(key).addPositions(start, end);
  }

  protected int[] getPositions(FeatureKey s) {
    tuple2<Integer, String> p = tuple2.valueOf(s.num, s.val);
...
//    }
  ...
  protected void add(int nFeatFrame, History history, int number) {
    tuple2<Integer,String> wT;
    int general = maxentTagger.extractors.size();

    if (nFeatFrame < general) {
      wT = tuple2.valueOf(nFeatFrame, maxentTagger.extractors.extract(nFeatFrame, history));
    } else {
      wT = tuple2.valueOf(nFeatFrame, maxentTagger.extractorsRare.extract(nFeatFrame - general, history));
    }
  ...
  protected int[] getXValues(tuple2<Integer, String> key) {
  ...
  public void save(DataOutputStream rf) {
    try {
      tuple2[] keys = new tuple2[tempHash.keySet().size()];
  ...
  private void read(InDataStreamFile rf) {
    try {
      ...
      for (int i = 0; i < numElem; i++) {
        ...
        tuple2<String,String> sWT = tuple2.readStringPair(rf);
        tuple2<Integer,String> wT = new tuple2<Integer,String>(Integer.parseInt(sWT.first()), sWT.second());
        ListInstances li = new ListInstances();
        li.read(rf);
        tempHash.put(wT, li);
      }// for
  ...
  public static void main(String[] args) {
    ...
    tuple2<Integer,String> p = new tuple2<Integer,String>(0, "0");
    ListInstances li = new ListInstances();
    li.add(14);
