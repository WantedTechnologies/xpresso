...
class TagCount {
  ...
  TagCount(Bag<String> tagCounts) {
    for (String tag : tagCounts) {
      map.put(tag, tagCounts.count(tag));
    }
   ...
   */
  public static TagCount readTagCount(DataInputStream rf) throws IOException{
    try {
      TagCount tc = new TagCount();
      int numTags = rf.readInt();
      tc.map = new HashMap<String, Integer>(numTags);
