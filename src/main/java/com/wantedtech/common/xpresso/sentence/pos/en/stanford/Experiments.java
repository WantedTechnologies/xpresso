...
public class Experiments {
  ...
  // todo [cdm 2013]: It might be better to change this to an tuple2<Integer,Integer>[]
  /**
  ...
  public Index<tuple2<Integer,Integer>> createIndex() {
    Index<tuple2<Integer,Integer>> index = new HashIndex<tuple2<Integer,Integer>>();
    for (int x = 0; x < px.length; x++) {
      int numberY = numY(x);
      for (int y = 0; y < numberY; y++) {
        index.add(tuple2.valueOf(x, y));
      }
    }
    return index;
  }
