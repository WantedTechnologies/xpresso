...
public class diff_match_patch {
   ...
   * e.g: The c<pre>at c</pre>ame. -&gt; The <pre>cat </pre>came.
   * @param diffs LinkedList of Diff objects.
   */
  ...
  public int match_main(String text, String pattern, int loc) {
    ...
    if (text.equals(pattern)) {
      ...
      // Do a fuzzy compare.
      return match_bitap(text, pattern, loc).get("location", Integer.class);
    }
  }
   ...
   * @return Best match index or -1.
   */
  protected tuple match_bitap(String text, String pattern, int loc) {
    ...
    return x.tuple(best_loc, best_score).name("location", "score");
  }
  
  double get_similarity_score (String one, String two) {
	  return Math.min(
			  1-(new diff_match_patch()).match_bitap(one, two, 0).get("score", Double.class)
			  ,
			  1-(new diff_match_patch()).match_bitap(two, one, 0).get("score", Double.class)
			 );
  }
   ...
   * @param diffs Array of Diff objects for text1 to text2.
   * @return LinkedList of Patch objects.
   * @deprecated Prefer patch_make(String text1, LinkedList{@code <Diff>} diffs).
   */
  public LinkedList<Patch> patch_make(String text1, String text2,
      LinkedList<Diff> diffs) {
    return patch_make(text1, diffs);
