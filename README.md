...
xpresso:
```
dict<Integer> coolCities = x.dict(x.yield("city","_").apply(x.chain(x.upper,x.strip,x.String.stripAccents)).replace(true).where("city","score").in(rank.items()).when(x.lambdaP("city, score : score > 20")));
```
